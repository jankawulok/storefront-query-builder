...
var RequestBody = /** @class */ (function () {
    ...
    RequestBody.prototype.buildQueryBodyFromSearchQuery = function () {
        if (this.searchQuery.getAppliedPostFilters().length > 0) {
            this.appliedPostFilters = clone_deep_1.default(this.searchQuery.getAppliedPostFilters());
     ...
     * @return {this}
     */
    RequestBody.prototype.applyPostFilters = function () {
        var _this = this;
        if (this.hasPostFilters()) {
            var postFilters = this.appliedPostFilters;
            var postFilterBuilder_1 = this.bodybuilder();
            postFilterBuilder_1.filterMinimumShouldMatch(1);
            postFilters.forEach(function (filter) {
                postFilterBuilder_1.filter('bool', function (catalogFilterQuery) {
                    return _this.catalogFilterBuilder(catalogFilterQuery, filter, undefined, 'orFilter');
                });
            });
            this.queryChain.rawOption('post_filter', postFilterBuilder_1.getFilter());
        }
        return this;
    };
    RequestBody.prototype.hasPostFilters = function () {
        if (!this._hasPostFilters) {
            this._hasPostFilters = this.searchQuery.getAppliedPostFilters().length > 0;
        }
        return this._hasPostFilters;
    };
    RequestBody.prototype.hasAggregations = function () {
    ...
    RequestBody.prototype.applyAggregations = function () {
        ...
        var postFilters = this.searchQuery.getAppliedPostFilters();
        var config = this.config.products;
        if (this.hasAggregations()) {
            var _loop_1 = function (attribute) {
                var postFilterBuilder = this_1.bodybuilder();
                postFilters.filter(function (f) { return f.attribute != attribute.field; }).forEach(function (filter) {
                    postFilterBuilder.filter('bool', function (catalogFilterQuery) {
                        return _this.catalogFilterBuilder(catalogFilterQuery, filter, undefined, 'orFilter');
                    });
                });
                if (attribute.type == 'terms') {
                    this_1.queryChain
                        .aggregation('filter', attribute.field, attribute.field, { _meta: { "type": attribute.type, "front_label": attribute.label } }, function (a) {
                        return (a.aggregation('terms', attribute.field, 'term', { size: attribute.size })
                ...
                if (attribute.type == 'range') {
                    this_1.queryChain
                        .aggregation('filter', attribute.field, attribute.field, function (a) {
                        return (a.aggregation('histogram', attribute.field, { _meta: { "type": attribute.type, "front_label": attribute.label }, "interval": 50 }, 'histogram').aggregation('stats', attribute.field, 'stats').filter('bool', postFilterBuilder.getFilter()["bool"]));
                    });
                }
            };
            var this_1 = this;
    ...
    RequestBody.prototype.applySort = function () {
        ...
        if (this.searchQuery.hasAppliedSort()) {
            ...
            var isBestMatchSort = appliedSort.some(function (f) { return f.field === 'best_match'; });
            if (isBestMatchSort) {
                var functions = { 'functions': [{ script_score: { script: "(1.5 * Math.max(0.0001, Math.log(Math.max(0.0001, (doc['sales'].empty ? 0 : doc['sales'].value + 2))))* (1.4 * Math.max(0.0001, Math.log(Math.max(0.0001,(doc['stock_qty'].empty ? 0 : doc['stock_qty'].value ))))))" } },
