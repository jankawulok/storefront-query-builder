    ...
    RequestBody.prototype.applyPostFilters = function () {
        ...
        if (this.hasPostFilters()) {
            ...
            postFilters.forEach(function (filter) {
                _this.catalogFilterBuilder(postFilterBuilder_1, filter, undefined, 'orFilter');
                postFilterBuilder_1.filter('bool', function (catalogFilterQuery) {
                    return _this.catalogFilterBuilder(catalogFilterQuery, filter, undefined, 'orFilter')
                        .orFilter('bool', function (b) { return _this.catalogFilterBuilder(b, filter, _this.optionsPrefix); });
                });
            });
    ...
    RequestBody.prototype.applyAggregations = function () {
        ...
        if (filters.length > 0) {
            var _loop_1 = function (attribute) {
                if (this_1.checkIfObjectHasScope({ object: attribute, scope: 'catalog' })) {
                    var field_1 = attribute.field;
                    var aggregationSize_1 = { size: config.filterAggregationSize[field_1] || config.filterAggregationSize.default };
                    var postFilterBuilder_2 = this_1.bodybuilder();
                    postFilters.filter(function (f) { return f.attribute != field_1; }).forEach(function (filter) {
                        postFilterBuilder_2.filter('bool', function (catalogFilterQuery) {
                            return _this.catalogFilterBuilder(catalogFilterQuery, filter, undefined, 'orFilter')
                                .orFilter('bool', function (b) { return _this.catalogFilterBuilder(b, filter, _this.optionsPrefix); });
                        });
                    });
                    if (field_1 !== 'price') {
                        this_1.queryChain
                            .aggregation('filter', field_1, function (a) {
                            return (a.aggregation('terms', _this.getMapping(field_1), aggregationSize_1).filter('bool', postFilterBuilder_2.getFilter()["bool"]));
                        })
                            .aggregation('terms', field_1 + this_1.optionsPrefix, aggregationSize_1).filter('bool', postFilterBuilder_2.getFilter()["bool"]);
                    }
                    else {
                        this_1.queryChain
                            .aggregation('terms', field_1).filter('bool', postFilterBuilder_2.getFilter()["bool"])
                            .aggregation('range', 'price', config.priceFilters).filter('bool', postFilterBuilder_2.getFilter()["bool"]);
                    }
                }
            };
            var this_1 = this;
