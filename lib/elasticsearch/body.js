        ...
        if (filters.length > 0) {
            var _loop_1 = function (attribute) {
                if (this_1.checkIfObjectHasScope({ object: attribute, scope: 'catalog' })) {
                    ...
                    if (field_1 !== 'price') {
                        this_1.queryChain
                            .aggregation('filter', field_1, field_1, { _meta: { "type": "terms", "front_label": "test1234" } }, function (a) {
                            return (a.aggregation('terms', field_1, 'term', aggregationSize_1)
                                .aggregation('cardinality', field_1, 'count')
                                .filter('bool', postFilterBuilder_2.getFilter()["bool"]));
                        });
                    }
                    else {
                        this_1.queryChain
                            .aggregation('filter', field_1, field_1, function (a) {
                            return (a.aggregation('histogram', field_1, { "interval": 50 }, 'histogram').aggregation('stats', field_1, 'stats').filter('bool', postFilterBuilder_2.getFilter()["bool"]));
                        });
                    }
                }
            };
